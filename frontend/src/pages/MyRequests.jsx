...
import { fetchMyRequests } from "../redux/requestSlice"; 
import {
  FaLaptop,
  FaMouse,
  FaKeyboard,
...
const MyRequests = () => {
  ...
  if (!Array.isArray(list) || list.length === 0) {
    return <p className="text-center text-gray-400 text-lg mt-12">No requests found</p>;
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-800 px-6 py-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {list.map((req) => (
          <div
            key={req._id}
            className="p-6 bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700 shadow-xl rounded-2xl hover:shadow-2xl transform hover:-translate-y-2 transition-all duration-300"
          >
            <div className="flex items-center justify-between mb-4">
              <div>{assetIcons[req.assetType?.toLowerCase()]}</div>
            ...
            <h3 className="font-bold text-2xl text-white mb-2">{req.assetType}</h3>
